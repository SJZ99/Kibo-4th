...
public class PathFinding {
    ...
    private void _decideRoute(int index, List<Integer> targets) {
        ...
        log("---------------------------");
        log("index: " + index);
        log("state: " + Integer.toBinaryString(wayPoints[index - 1].getVisited()));
//        log("previous id: " + wayPoints[index - 1].getId());
        // used all targets, or has used point 8 (goal)
        if(wayPoints[index - 1].numberOfVisitedPoints() == targets.size() + 1 || wayPoints[index - 1].getId() == 8) {
...
//                log("have contained: id=" + i);
            ...
            log("path time: " + pathTime);
//            log("phrase time: " + phraseTime);
...
//            log("remove (id): " + wayPoints[index].getId());

            // or, construct a route without this target
            if(i != 7 && i != 8) {
